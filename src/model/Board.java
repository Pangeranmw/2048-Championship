...
public class Board extends Entity{
	...
	public Board(int x, int y) {
		...
		scores.loadGame();
		scores.setBestTime(Leaderboards.getInstance().getFastestTime());
		scores.setCurrentTopScore(Leaderboards.getInstance().getHighScore());
		if(scores.newGame()){
			start();
			scores.saveGame();
		}
	...
	private void spawnRandom() {
		...
		while (notValid) {
			...
			if (current == null) {
				int value=0;
				if(Level.getInstance().getLevel()==1) {
					value = random.nextInt(2) < 1 ? 16 : 8;
				}else if(Level.getInstance().getLevel()==2) {
					value = random.nextInt(2) < 1 ? 8 : 4;
				} else if(Level.getInstance().getLevel()==3) {
	...
	private void checkKeys() {
		if (!Keys.pressed[KeyEvent.VK_LEFT] && Keys.lastPressed[KeyEvent.VK_LEFT]) {
			moveTiles(LEFT);
			if (!hasStarted) hasStarted = !dead;
		}
		if (!Keys.pressed[KeyEvent.VK_RIGHT] && Keys.lastPressed[KeyEvent.VK_RIGHT]) {
			moveTiles(RIGHT);
			if (!hasStarted) hasStarted = !dead;
		}
		if (!Keys.pressed[KeyEvent.VK_UP] && Keys.lastPressed[KeyEvent.VK_UP]) {
			moveTiles(UP);
			if (!hasStarted) hasStarted = !dead;
		}
		if (!Keys.pressed[KeyEvent.VK_DOWN] && Keys.lastPressed[KeyEvent.VK_DOWN]) {
			moveTiles(DOWN);
			if (!hasStarted) hasStarted = !dead;
		}
	}
	...
	public void setWon(boolean won) {
		if(!isWon() && won && !dead){
                    long banding = (scores.getBestTime()==0) ? Long.MAX_VALUE : scores.getBestTime();
                    
                    Leaderboards.getInstance().addTime(scores.getTime());
                    if(scores.getTime()<banding) {
	...
	public void setDead(boolean dead) {
		if(!isDead() && dead){
			Leaderboards.getInstance().addTile(getHighestTileValue());
			Leaderboards.getInstance().addScore(scores.getCurrentScore());
			Leaderboards.getInstance().addLeaderboards(this);
		}
		this.dead = dead;
	}
